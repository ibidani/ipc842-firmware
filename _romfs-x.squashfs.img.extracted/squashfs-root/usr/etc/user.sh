#! /bin/sh
echo 50 > /proc/sys/vm/dirty_writeback_centisecs # cache刷新周期改为1s
echo 1 > /proc/sys/vm/dirty_expire_centisecs # cache超时时间改为5s
echo 50 > /proc/sys/vm/vfs_cache_pressure # 倾向于保留directory和inode cache
echo 30 > /proc/sys/vm/swappiness # 降低磁盘交换程度
echo 1 > /proc/sys/vm/dirty_ratio # 进程脏数据达到1%以上时，自行写回数据
echo 1 > /proc/sys/vm/dirty_background_ratio # 脏数据达到1%以上时，自行写回数据
